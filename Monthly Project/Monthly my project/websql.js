...
(function () {
    ...
    var gid;
    var records;
    var db = openDatabase('MyContacts3', '2.0', 'MyContacts3', 5 * 1024 * 1024);
    db.changeVersion('2.0', '2.0', migrateDB, onError, onSuccess);
    function migrateDB(transaction) {
        transaction.executeSql("CREATE TABLE IF NOT EXISTS contact3(" +
    ...
    function migrateDB1(transaction) {
        console.log("INSERT INTO contact3 VALUES(" + $("#id").val() + ",'" + $("#firstName").val() + "', '" + $("#lastName").val() + "','" + $("#email").val() + "','" + $("#phoneNumber").val() + "','" + $("#imgshow").html() + "')");
        transaction.executeSql("INSERT INTO contact3 VALUES(" + $("#id").val() + ",'" + $("#firstName").val() + "', '" + $("#lastName").val() + "','" + $("#email").val() + "','" + $("#phoneNumber").val() + "','" + $("#imgshow").html() + "')");
    };
    ns.save = function () {
        db.changeVersion('2.0', '2.0', migrateDB1, onInsertError, onSuccess);
        ns.display();
    ...
    function onInsertError(error) {
        if (error.code == 6) {
            db.changeVersion('2.0', '2.0', migrateDB4, onError, onSuccess);
            ns.display();
        }
        else
            alert("Error code: " + error.code + " Message: " + error.message);
    ...
    function migrateDB2(transaction) {
        transaction.executeSql("SELECT * FROM contact3", [], bindToGrid)
    }
    function migrateDB3(transaction, id) {
        transaction.executeSql("SELECT * FROM contact3 where id=?", [gid], showinTexts)
    }
    function migrateDB4(transaction) {
        transaction.executeSql("update contact3 set firstname='" + $("#firstName").val() + "',lastname= '" + $("#lastName").val() + "',email='" + $("#email").val() + "',phonenumber='" + $("#phoneNumber").val() + "',logo='" + $("#logo").html() + "' where id=" + $("#id").val());
    }
    function migrateDB5(transaction) {
        alert("delete from contact2  where id=" + gid);
        transaction.executeSql("delete from contact3  where id=" + gid);
    }
    ...
    ns.loadContact = function () {
        gid = parseInt($(this).attr('data-key'));
        db.changeVersion('2.0', '2.0', migrateDB3, onError, onSuccess);
